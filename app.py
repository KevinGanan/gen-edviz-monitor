...
```
    ...
    with col_config:
        ...
        tab_manual, tab_plantilla, tab_mejorar = st.tabs([
            "✍️ Prompt Manual", 
            "📋 Usar Plantilla", 
            "✨ Mejorar con IA"
        ])
        ...
        # ========== TAB: PLANTILLA ==========
        with tab_plantilla:
            st.markdown("Selecciona una plantilla predefinida para estructuras de datos.")
            
            temas_disponibles = listar_temas_disponibles()
            
            tema_plantilla = st.selectbox(
                "🏷️ Selecciona el tema",
                options=["(Seleccionar)"] + temas_disponibles,
                key="gen_tema_plantilla"
            )
            
            if tema_plantilla and tema_plantilla != "(Seleccionar)":
                conceptos_disponibles = listar_conceptos_por_tema(tema_plantilla)
                
                concepto_plantilla = st.selectbox(
                    "📌 Selecciona el concepto",
                    options=["(Seleccionar)"] + conceptos_disponibles,
                    format_func=lambda x: x.replace("_", " ").title() if x != "(Seleccionar)" else x,
                    key="gen_concepto_plantilla"
                )
                
                if concepto_plantilla and concepto_plantilla != "(Seleccionar)":
                    plantilla = obtener_plantilla(tema_plantilla, concepto_plantilla)
                    
                    st.markdown("**Prompt de la plantilla:**")
                    prompt_plantilla_edit = st.text_area(
                        "Puedes editarlo si deseas",
                        value=plantilla,
                        height=120,
                        key="gen_prompt_plantilla_edit"
                    )
                else:
                    prompt_plantilla_edit = ""
            else:
                concepto_plantilla = ""
                prompt_plantilla_edit = ""
        ...
        elif 'gen_prompt_plantilla_edit' in st.session_state and st.session_state.get("gen_prompt_plantilla_edit", "").strip():
            prompt_final = st.session_state.get("gen_prompt_plantilla_edit", "").strip()
            tema_final = tema_plantilla if tema_plantilla != "(Seleccionar)" else ""
            concepto_final = concepto_plantilla if concepto_plantilla != "(Seleccionar)" else ""
        ...
        if prompt_final and tema_final and concepto_final:
            with st.expander("💡 Sugerencias para mejor resultado", expanded=True):
                st.caption("Estos prompts están optimizados para GPT-Image 1.5:")
                
                sugerencias = sugerir_prompt_educativo(tema_final, concepto_final)
                
                if sugerencias:
                    tabs = st.tabs([f"Sugerencia {i+1}" for i in range(min(3, len(sugerencias)))])
                    
                    for i, tab in enumerate(tabs):
                        with tab:
                            sugerencia = sugerencias[i]
                            st.code(sugerencia, language="text")
                            
                            if st.button("Usar esta sugerencia", key=f"use_suggestion_{i}"):
                                if prompt_manual and prompt_manual.strip():
                                    st.session_state["prompt_suggestion_applied"] = True
                                    st.session_state["suggested_prompt"] = sugerencia
                                    st.session_state["suggestion_for_tab"] = "manual"
                                elif tema_plantilla != "(Seleccionar)":
                                    st.session_state["prompt_suggestion_applied"] = True
                                    st.session_state["suggested_prompt"] = sugerencia
                                    st.session_state["suggestion_for_tab"] = "plantilla"
                                elif st.session_state.prompt_mejorado:
                                    st.session_state["prompt_suggestion_applied"] = True
                                    st.session_state["suggested_prompt"] = sugerencia
                                    st.session_state["suggestion_for_tab"] = "mejorar"
                                
                                st.success("Sugerencia seleccionada.")
                                st.rerun()
                    
                    if st.session_state.get("prompt_suggestion_applied", False):
                        if st.session_state.get("suggestion_for_tab", "") == "manual":
                            st.text_area(
                                "💬 Prompt para GPT-Image 1.5",
                                value=st.session_state.get("suggested_prompt", prompt_final),
                                height=150,
                                key="gen_prompt_manual_updated"
                            )
                        elif st.session_state.get("suggestion_for_tab", "") == "plantilla":
                            st.text_area(
                                "Puedes editarlo si deseas",
                                value=st.session_state.get("suggested_prompt", prompt_final),
                                height=120,
                                key="gen_prompt_plantilla_edit_updated"
                            )
                        elif st.session_state.get("suggestion_for_tab", "") == "mejorar":
                            st.text_area(
                                "Puedes editarlo antes de generar",
                                value=st.session_state.get("suggested_prompt", prompt_final),
                                height=120,
                                key="gen_prompt_mejorado_edit_updated"
                            )
                        
                        del st.session_state["prompt_suggestion_applied"]
                        del st.session_state["suggested_prompt"]
                        del st.session_state["suggestion_for_tab"]
                else:
                    st.info("No hay sugerencias específicas para este tema y concepto.")
                
                st.caption("💡 Los prompts técnicos y específicos generan mejores imágenes educativas.")
            ...
            with col_act1:
                if st.button("📥 Guardar en Catálogo", type="primary", key="btn_guardar_catalogo"):
                    with st.spinner("Guardando y calculando métricas..."):
                        ...
                        carpeta = Path("data/catalogo_imagenes")
                        carpeta.mkdir(parents=True, exist_ok=True)

                        # Guardar archivo físico
                        ruta_imagen = carpeta / f"{image_id}.png"
                        ...
                        try:
                            ...
                            st.warning(f"⚠️ No se pudieron calcular métricas: {e}")
                            wcag_val = 0
                            edge_val = 0

                        # Preparar registro para CSV
                        ...
                        nueva_fila = {
                            ...
                            "wcag_ratio": round(wcag_val, 2),  # ✅ CON VALOR
                            "edge_density": round(edge_val, 3),  # ✅ CON VALOR
                            "estado": "pendiente",  # ✅ ESTADO INICIAL
                            "version": "v1",  # ✅ VERSIÓN
                            "autor": user_name,  # ✅ AUTOR
                            "alt_text": img_data.get("revised_prompt", "")[:200],  # ✅ ALT TEXT
                            "fecha": datetime.now().strftime("%Y-%m-%d %H:%M")
                        }

                        if catalogo_path.exists():
                            df_catalogo = pd.read_csv(catalogo_path, encoding="utf-8")
                            df_catalogo = pd.concat([df_catalogo, pd.DataFrame([nueva_fila])], ignore_index=True)
                        else:
                            df_catalogo = pd.DataFrame([nueva_fila])

                        df_catalogo.to_csv(catalogo_path, index=False, encoding="utf-8")

                        st.success(f"✅ Imagen guardada | WCAG: {wcag_val:.2f} | Bordes: {edge_val:.3f}")
...
# -------------------------------------------------------
    ...
    with col_img:
        ...
        catalogo_fresh = pd.read_csv(Path("data/catalogo.csv"), encoding="utf-8")
        row = catalogo_fresh[catalogo_fresh["image_id"] == selected_id].iloc[0]
        
        # Mostrar imagen
        ruta = _safe_path(row.get("ruta", ""))
        if ruta and Path(ruta).exists():
            st.image(ruta, caption=f"{row['tema']} / {row['concepto']}")
        else:
                ...
                if st.button("💾 Guardar Evaluación"):
                    try:
                        ...
                        for msg in st.session_state.ia_chat_history:
                            ...
                            if 'coherencia' in contenido and any(c.isdigit() for c in contenido):
                                import re
                                nums = re.findall(r'coherencia[:\s]*(\d+(?:\.\d+)?)', contenido)
                                if nums:
                                    coherencia = float(nums[0])
                            if 'fidelidad' in contenido and any(c.isdigit() for c in contenido):
                                nums = re.findall(r'fidelidad[:\s]*(\d+(?:\.\d+)?)', contenido)
                                if nums:
                                    fidelidad = float(nums[0])
                            if 'claridad' in contenido and any(c.isdigit() for c in contenido):
                                nums = re.findall(r'claridad[:\s]*(\d+(?:\.\d+)?)', contenido)
                                if nums:
                                    claridad = float(nums[0])
...
{prompt}
                ...
                ia_eval_path = Path("data/ia_evaluations/ia_eval.csv")
                
                if ia_eval_path.exists():
                    try:
                        df_ia_eval = pd.read_csv(ia_eval_path, encoding="utf-8")
                        eval_imagen = df_ia_eval[df_ia_eval["image_id"] == img_id]
                    ...
                    st.markdown("""
                    <div style="background: rgba(100, 116, 139, 0.1); border-radius: 12px; padding: 20px;
                                border: 1px dashed rgba(100, 116, 139, 0.3); text-align: center;">
                        <div style="font-size: 2rem; margin-bottom: 10px;">📋</div>
                        <div style="color: #94a3b8; font-weight: 500;">No hay evaluaciones registradas</div>
                    </div>
                    """, unsafe_allow_html=True)
                        ...
                        with del_cols[0]:
                            if st.button("Sí, eliminar", key="confirm_del_yes", type="primary"):
                                try:
                                    ...
                                    if p and p.exists():
                                        p.unlink()
                                    df_meta = df_meta[df_meta["image_id"] != img_id].reset_index(drop=True)
                                    save_csv(df_meta, META_PATH)
        ...
        with col1:
            ...
            with c_center:
                ...
                if ia_eval_path.exists():
                    try:
                        df_ia_eval = pd.read_csv(ia_eval_path, encoding="utf-8")
                        eval_imagen = df_ia_eval[df_ia_eval["image_id"] == meta["image_id"]]
                        ...
                        if not eval_imagen.empty:
                            ...
                            # --- RECOMENDACIONES ---
                            recomendaciones = eval_row.get('recomendaciones', '')
                            if recomendaciones and str(recomendaciones).strip().lower() not in ['nan', '']:
                                with st.expander("💡 Ver conversación completa con IA"):
                                    st.markdown(f"""
                                    <div style="background: rgba(34, 197, 94, 0.1); border-radius: 10px; padding: 14px;
                                                border-left: 3px solid #22c55e;">
                                        <div style="color: #86efac; line-height: 1.6; white-space: pre-wrap;">
                            ...
                            st.info("Esta imagen aún no tiene evaluación IA. Ve a 'Evaluación IA (imagen + prompt)' para evaluarla.")

                    except Exception as e:
                        st.error(f"Error cargando evaluación IA: {e}")

                else:
                    st.info("No existe archivo de evaluaciones IA.")
...
# -------------------------------------------------------
    ...
    if df_meta.empty:
        ...
        with col_m3:
            ...
            if ia_eval_path.exists():
                df_ia_eval = pd.read_csv(ia_eval_path, encoding="utf-8")
                evaluadas_ia = len(df_ia_eval['image_id'].unique())
                    ...
                    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as z:
                        ...
                        for _, row in aceptadas.iterrows():
                            ...
                            if img_path.exists():
                                ...
                                archivo_en_zip = f"imagenes/{img_path.name}"
                                z.writestr(archivo_en_zip, img_data)
